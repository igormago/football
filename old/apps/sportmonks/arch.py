...
from keras.layers import LSTM, Dropout, Permute, Conv1D, BatchNormalization, Activation, Masking, \
    GlobalAveragePooling1D, concatenate, Dense, regularizers, constraints, Reshape, multiply, Recurrent, activations, \
    initializers, K, interfaces, Bidirectional
import tensorflow as tf

def mlst_fcn(n_timesteps, n_features, n_outputs):

    MAX_TIMESTEPS = n_timesteps
    MAX_NB_VARIABLES = n_features
    NB_CLASS = n_outputs
...
def malstm_fcn(n_timesteps, n_features, n_outputs):
    MAX_TIMESTEPS = n_timesteps
    MAX_NB_VARIABLES = n_features
    NB_CLASS = n_outputs
...
def lstm_fcn (n_timesteps, n_features, n_outputs):
    MAX_TIMESTEPS = n_timesteps
    MAX_NB_VARIABLES = n_features
    NB_CLASS = n_outputs
...
def alstm_fcn(n_timesteps, n_features, n_outputs):
...
def josif (n_timesteps, n_features, n_outputs):
    ...
    y = Bidirectional(LSTM(2))(y)

    out = Dense(NB_CLASS)(y)
    model = Model(ip, out)
...
def dense_1_layer(input_dim=17, n_filters=[4], dropout=0):
    model = Sequential()
    model.add(Dense(n_filters[0], input_dim=input_dim))
    model.add(Activation('relu'))
    model.add(Dropout(dropout))

    for i in n_filters[1:]:
        model.add(Dense(i))
        model.add(Activation('relu'))
        model.add(Dropout(dropout))
...
def dense_1_layer_n(input_dim=17, n_filters=[4], dropout=0):
