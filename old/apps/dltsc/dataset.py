...
class Dataset:
    ...
    def load_multivariate(self, dataset_prefix, crop):
    ...
    def load_ucr_univariate_data(self, dataset_folder=None):
        ...
        train_data = np.loadtxt(file_prefix + "_TRAIN.tsv", delimiter="\t")
        test_data = np.loadtxt(file_prefix + "_TEST.tsv", delimiter="\t")
        ...
        print("Train series shape", self.X_train.shape)
        print("Test series shape", self.X_test.shape)
